...
public class FakeCraftingGrid implements CraftingGrid {
	...
	public boolean setStack(int slot, Optional<Item> item) {
		...
		Optional<Item> orig = getStack(slot);
		if (orig.equals(item)) {
			return false;
		}
	...
	public boolean setStack(int x, int y, Optional<Item> item) {
		OptionalInt slot = coordsToSlot(x, y);
		if (slot.isPresent()) {
			return setStack(slot.getAsInt(), item);
		} else {
			return false;
		}
	}
	...
	public String toString() {
		...
		for (int y = 0; y < getHeight(); y++) {
			StringJoiner sj = new StringJoiner("|", "|", "|");
			for (int x = 0; x < getWidth(); x++) {
				sj.add(String.format(itemFormat, getStack(x, y).map(Identifiable::getID).orElse("<null>")));
			}
