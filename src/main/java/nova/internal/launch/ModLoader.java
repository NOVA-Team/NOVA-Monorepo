	...
	public void preInit() {
		orderedMods.stream().forEachOrdered(mod -> {
			try {
				...
				Game.getInstance().logger().error("Critical error caught during pre initialization phase", t);
	...
	public void init() {
		orderedMods.stream().forEachOrdered(mod -> {
			try {
				...
				Game.getInstance().logger().error("Critical error caught during initialization phase", t);
				throw new NovaException(t);
			}
		});
	}
	...
	public void postInit() {
		orderedMods.stream().forEachOrdered(mod -> {
			try {
				...
				Game.getInstance().logger().error("Critical error caught during post initialization phase", t);
				throw new NovaException(t);
			}
		});
	}
