...
public interface ItemWrapperMethods extends IItemRenderer {
	...
	default void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean p_77624_4_) {
		Item item = getItemFactory()
			.makeItem(DataUtility.nbtToData(itemStack.getTagCompound()));
	...
	default boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		Item item = getItemFactory().makeItem(DataUtility.nbtToData(itemStack.getTagCompound())).setCount(itemStack.stackSize);
	...
	default IIcon getIcon(ItemStack itemStack, int pass) {
		if (getItemFactory().makeItem(DataUtility.nbtToData(itemStack.getTagCompound())).setCount(itemStack.stackSize).getTexture().isPresent()) {
			return RenderUtility.instance.getIcon(getItemFactory().makeItem(DataUtility.nbtToData(itemStack.getTagCompound())).setCount(itemStack.stackSize).getTexture().get());
		}
		return null;
	}
	...
	default void renderItem(IItemRenderer.ItemRenderType type, ItemStack itemStack, Object... data) {
		getItemFactory().makeItem(DataUtility.nbtToData(itemStack.getTagCompound())).setCount(itemStack.stackSize).onRender(type.ordinal(), data);
	}

	default int getColorFromItemStack(ItemStack itemStack, int p_82790_2_) {
		return getItemFactory().makeItem(DataUtility.nbtToData(itemStack.getTagCompound())).setCount(itemStack.stackSize).colorMultiplier().argb();
	}
}