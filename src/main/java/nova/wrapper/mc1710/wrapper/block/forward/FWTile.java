...
public class FWTile extends TileEntity {
	...
	public Packet getDescriptionPacket() {
		if (block instanceof nova.core.network.PacketHandler) {
			return ((MCNetworkManager) Game.networkManager()).toMCPacket(((MCNetworkManager) Game.networkManager()).getBlockPacket(0, (nova.core.network.PacketHandler) block));
		}
		return null;
	}
	...
	public void writeToNBT(NBTTagCompound nbt) {
		...
		if (block != null) {
			if (block instanceof Storable) {
				...
				nbt.setTag("nova", Game.nativeManager().toNative(data));
			}
		}
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		cacheData = Game.nativeManager().toNova(nbt.getCompoundTag("nova"));
	}
}