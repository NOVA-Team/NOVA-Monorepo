...
public class RenderUtility {
	...
	public void preTextureHook(TextureStitchEvent.Pre event) {
		if (event.map.getTextureType() == 0) {
			Game.renderManager().blockTextures.forEach(t -> registerIcon(t, event));
		} else if (event.map.getTextureType() == 1) {
			Game.renderManager().itemTextures.forEach(t -> registerIcon(t, event));
		}
	}
	...
	public void loadModels() {
		Game.renderManager().modelProviders.forEach(m -> {
