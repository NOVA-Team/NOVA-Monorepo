...
public class WavefrontObjectModelProvider extends ModelProvider {
	...
	private VertexModel currentModel = null;
			...
			while ((currentLine = reader.readLine()) != null) {
				...
				if (currentLine.startsWith("#") || currentLine.isEmpty()) {
					...
					if (currentModel == null) {
						currentModel = new VertexModel("Default");
					}
					...
					VertexModel subModel = parseToModel(currentLine, lineCount);
					if (subModel != null) {
						if (currentModel != null) {
	...
	private VertexModel parseToModel(String line, int lineNumber) {
		if (isValid(line, subModelPattern)) {
			String trimmedLine = line.substring(line.indexOf(" ") + 1);
			if (!trimmedLine.isEmpty()) {
				return new VertexModel(trimmedLine);
			}
		} else {
			throw new RenderException("Error parsing entry ('" + line + "'" + ", line " + lineNumber + ") in model '" + name + "' - Incorrect format");
		}
