...
public class TEBlockTransform extends BlockTransform {
	...
	public void setWorld(World world) {
		...
		((FWTile) WorldConverter.instance().toNative(world).getTileEntity(blockPos())).setBlock(tileEntity.block);
	}
	...
	public void setPosition(Vector3D position) {
		world().setBlock(position, tileEntity.block.getFactory());
		world().removeBlock(position());
		((FWTile) blockAccess().getTileEntity(VectorConverter.instance().toNative(position))).setBlock(tileEntity.block);
	}
}
